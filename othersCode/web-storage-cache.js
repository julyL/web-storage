...
(function (root, factory) {
    ...
    */
    ...
    function CacheItemConstructor (value, exp) {
        // createTime
        this.c = (new Date()).getTime();   // 设置起始时间
        exp = exp || _defaultExpire;
        var expires = _getExpiresDate(exp);
        // expiresTime
        this.e = expires.getTime();       // 设置过期时间
        this.v = value;                   // 设置value
    }
    ...
    var CacheAPIImpl = {
        ...
        set: function(key, val, options) {
            ...
            key = _checkAndWrapKeyAsString(key);  // key必须为String

            options = _extend({force: true}, options);

            if (val === undefined) {   // 如果设置的val为undefined,则删除
                return this.delete(key);
            }

            var value = defaultSerializer.serialize(val);  // 处理val值(默认进行JSON.stringify)
